#Script
    param (
        ...
        $SVV4H72ConnServerRMIName = "V4H to View Connection Server RMI",
        $SVV4H72ConnServerBMSName = "V4H to View Connection Server Broker Message Server",
        $SVV4H72ConnServerCMSName = "V4H to View Connection Server Cert Management Server",
        $SVV4H72AgentRMIName = "V4H to Horizon Agent RMI",
        $SVV4H72AgentDMSName = "V4H to Horizon Agent Desktop Message Server",
        $SVV4H72UAGMonitorName = "V4H to Unified Access Gateway Monitoring",
        $SVV4H72APPVMGRMonitorName = "V4H to App Volumes Manager Monitoring",
        ...
        $SVHZN7BEClient2AgentTCP = New-NsxService -name $SVHZNBEClient2AgentTCPName -protocol $TCP -port $22443 -description "Horizon 7 Blast Extreme TCP Excellent Typical Horizon Client to Horizon Agent" -EnableInheritance
        $SVHZN7BEClient2AgentUDP = New-NsxService -name $SVHZNBEClient2AgentUDPName -protocol $UDP -port $22443 -description "Horizon 7 Blast Extreme UDP Typical Horizon Client to Horizon Agent" -EnableInheritance
        $SVHZN7PCOIPClient2AgentTCP = New-NsxService -name $SVHZNPCOIPClient2AgentTCPName -protocol $TCP -port $4172 -description "Horizon 7 PCoIP TCP Horizon Client to Horizon Agent" -EnableInheritance
        $SVHZN7PCOIPClient2AgentUDP = New-NsxService -name $SVHZNPCOIPClient2AgentUDPName -protocol $UDP -port $4172 -description "Horizon 7 PCoIP UDP Horizon Client to Horizon Agent" -EnableInheritance
        $SVHZN7RDPClient2Agent = New-NsxService -name $SVHZNRDPClient2AgentName -protocol $TCP -port $3389 -description "Horizon 7 RDP Horizon Client to Horizon Agent" -EnableInheritance
        $SVHZN7CDRMMRClient2Agent = New-NsxService -name $SVHZNCDRMMRClient2AgentName -protocol $TCP -port $9427 -description "Horizon 7 CDR MMR Horizon Client to Horizon Agent" -EnableInheritance
        $SVHZN7USBClient2Agent = New-NsxService -name $SVHZNUSBClient2AgentName -protocol $TCP -port $32111 -description "Horizon 7 USB Horizon Client to Horizon Agent USB Redirection" -EnableInheritance
        $SVHZN7Browser2AgentHTML = New-NsxService -name $SVHZNBrowser2AgentHTMLName -protocol $TCP -port $443 -description "Horizon 7 Browser to Horizon Agent HTML Access" -EnableInheritance
        $SVHZN7Browser2VIDM = New-NsxService -name $SVHZNBrowser2VIDMName -protocol $TCP -port $443 -description "Horizon 7 Browser to VMware Identity Manager" -EnableInheritance
        $SVHZN7Client2UAG = New-NsxService -name $SVHZNClient2UAGName -protocol $TCP -port $443 -description "Horizon 7 Horizon Client to Unified Access Gateway" -EnableInheritance
        $SVHZN7PCOIPClient2UAGTCP = New-NsxService -name $SVHZNPCOIPClient2UAGTCPName -protocol $TCP -port $4172 -description "Horizon 7 PCoIP TCP Horizon Client to Unified Access Gateway" -EnableInheritance
        $SVHZN7PCOIPClient2UAGUDP = New-NsxService -name $SVHZNPCOIPClient2UAGUDPName -protocol $TCP -port $4172 -description "Horizon 7 PCoIP UDP Horizon Client to Unified Access Gateway" -EnableInheritance
        $SVHZN7BEClient2UAGTCP443 = New-NsxService -name $SVHZNBEClient2UAGTCP443Name -protocol $TCP -port $443 -description "Horizon 7 Blast Extreme TCP 443 Excellent Typical Horizon Client to Unified Access Gateway" -EnableInheritance
        $SVHZN7BEClient2UAGTCP8443 = New-NsxService -name $SVHZNBEClient2UAGTCP8443Name -protocol $TCP -port $8443 -description "Horizon 7 Blast Extreme TCP 8443 Excellent Typical Horizon Client to Unified Access Gateway" -EnableInheritance
        $SVHZN7BEClient2UAGUDP443 = New-NsxService -name $SVHZNBEClient2UAGUDP443Name -protocol $UDP -port $8443 -description "Horizon 7 Blast Extreme UDP 443 Poor Typical Horizon Client to Unified Access Gateway" -EnableInheritance
        $SVHZN7BEClient2UAGUDP8443 = New-NsxService -name $SVHZNBEClient2UAGUDP8443Name -protocol $UDP -port $443 -description "Horizon 7 Blast Extreme UDP 8443 Poor Typical Horizon Client to Unified Access Gateway" -EnableInheritance
        $SVHZN7Browser2UAGHTML = New-NsxService -name $SVHZNBrowser2UAGHTMLName -protocol $TCP -port $443 -description "Horizon 7 Browser to Unified Access Gateway HTML Access" -EnableInheritance
        $SVHZN7Browser2UAGVIDM = New-NsxService -name $SVHZNBrowser2UAGVIDMName -protocol $TCP -port $443 -description "Horizon 7 Browser to Unified Access Gateway VIDM" -EnableInheritance
        $SVHZN7Client2ConnServerTunnHTTPS = New-NsxService -name $SVHZNClient2ConnServerTunnHTTPSName -protocol $TCP -port $443 -description "Horizon 7 HTTPS Horizon Client to Connection Servers Tunneled" -EnableInheritance
        $SVHZN7Client2ConnServerTunnHTTP = New-NsxService -name $SVHZNClient2ConnServerTunnHTTPName -protocol $TCP -port $80 -description "Horizon 7 HTTP Horizon Client to View Connection Servers Tunneled" -EnableInheritance
        $SVHZN7BEClient2ConnServerTCP = New-NsxService -name $SVHZNBEClient2ConnServerTCPName -protocol $TCP -port $8443 -description "Horizon 7 Blast Extreme TCP Excellent Typical Horizon Client to View Connection Server" -EnableInheritance
        $SVHZN7PCOIPClient2ConnServerTCP = New-NsxService -name $SVHZNPCOIPClient2ConnServerTCPName -protocol $TCP -port $4172 -description "Horizon 7 PCoIP TCP Horizon Client to View Connection Server" -EnableInheritance
        $SVHZN7PCOIPClient2ConnServerUDP = New-NsxService -name $SVHZNPCOIPClient2ConnServerUDPName -protocol $UDP -port $4172 -description "Horizon 7 PCoIP UDP Horizon Client to View Connection Server" -EnableInheritance
        $SVHZN7Browser2AgentTunnHTML = New-NsxService -name $SVHZNBrowser2AgentTunnHTMLName -protocol $TCP -port $443 -description "Horizon 7 Browser to Horizon Agent HTML Access Tunneled" -EnableInheritance
        $SVHZN7Browser2VIDMTunn = New-NsxService -name $SVHZNBrowser2VIDMTunnName -protocol $TCP -port $443 -description "Horizon 7 Browser to VMware Identity Manager Tunneled" -EnableInheritance
        $SVHZN7Agent2ConnServerEnhanced = New-NsxService -name $SVHZNAgent2ConnServerEnhancedName -protocol $TCP -port $4002 -description "Horizon 7 JMS Horizon Agent to Connection Server Enhanced" -EnableInheritance
        $SVHZN7Agent2ConnServerLegacy = New-NsxService -name $SVHZNAgent2ConnServerLegacyName -protocol $TCP -port $4001 -description "Horizon 7 JMS Horizon Agent to Connection Server Legacy" -EnableInheritance
        $SVHZN7Agent2V4HRMI = New-NsxService -name $SVHZNAgent2V4HRMIName -protocol $TCP -port $3091 -description "Horizon 7 Horizon Agent to V4H RMI" -EnableInheritance
        $SVHZN7Agent2V4HDMS = New-NsxService -name $SVHZNAgent2V4HDMSName -protocol $TCP -port $3099 -description "Horizon 7 Horizon Agent to V4H Desktop Message Server" -EnableInheritance
        ...
        $SVHZN7BEConnServ2AgentTunn = New-NsxService -name $SVHZNBEConnServ2AgentTunnName -protocol $TCP -port $22443 -description "Horizon 7 Blast Extreme TCP View Connection Server to Horizon Agent Tunneled" -EnableInheritance
        $SVHZN7PCOIPConnServ2AgentTunnTCP = New-NsxService -name $SVHZNPCOIPConnServ2AgentTunnTCPName -protocol $TCP -port $4172 -description "Horizon 7 PCoIP TCP View Connection Server to Horizon Agent Tunneled" -EnableInheritance
        $SVHZN7PCOIPConnServ2AgentTunnUDP = New-NsxService -name $SVHZNPCOIPConnServ2AgentTunnUDPName -protocol $UDP -port $4172 -description "Horizon 7 PCoIP UDP View Connection Server to Horizon Agent Tunneled" -EnableInheritance
        $SVHZN7RDPConnServer2AgentTunn = New-NsxService -name $SVHZNRDPConnServer2AgentTunnName -protocol $TCP -port $3389 -description "Horizon 7 RDP View Connection Server to Horizon Agent Tunneled" -EnableInheritance
        $SVHZN7CDRMMRConnServer2AgentTunn = New-NsxService -name $SVHZNCDRMMRConnServer2AgentTunnName -protocol $TCP -port $9427 -description "Horizon 7 CDR MMR View Connection Server to Horizon Agent Tunneled" -EnableInheritance
        $SVHZN7USBClient2AgentUSBTunn = New-NsxService -name $SVHZNUSBClient2AgentUSBTunnName -protocol $TCP -port $32111 -description "Horizon 7 USB Horizon Client to Horizon Agent USB Redirection Tunneled" -EnableInheritance
        $SVHZN7ConnServer2vCenterSOAP = New-NsxService -name $SVHZNConnServer2vCenterSOAPName -protocol $TCP -port $433 -description "Horizon 7 HTTPS View Connection Server to vCenter Server SOAP" -EnableInheritance
        $SVHZN7ConnServer2CompSOAP = New-NsxService -name $SVHZNConnServer2CompSOAPName -protocol $TCP -port $18443 -description "Horizon 7 View Connection Server to View Composer SOAP" -EnableInheritance
        $SVHZN7ConnServer2ConnServerJMSLegacy = New-NsxService -name $SVHZNConnServer2ConnServerJMSLegacyName -protocol $TCP -port $4100 -description "Horizon 7 JMS View Connection Server to View Connection Server Legacy" -EnableInheritance
        $SVHZN7ConnServer2ConnServerJMSSSL = New-NsxService -name $SVHZNConnServer2ConnServerJMSSSLName -protocol $TCP -port $4101 -description "Horizon 7 JMS View Connection Server to View Connection Server SSL" -EnableInheritance
        $SVHZN7CPALDAPREPL = New-NsxService -name $SVHZNCPALDAPREPLName -protocol $TCP -port $22389 -description "Horizon 7 CPA Global LDAP Replication" -EnableInheritance
        $SVHZN7CPALDAPREPLSSL = New-NsxService -name $SVHZNCPALDAPREPLSSLName -protocol $TCP -port $22636 -description "Horizon 7 CPA Global LDAP Replication SSL" -EnableInheritance
        $SVHZN7CPAInterPodVIPA = New-NsxService -name $SVHZCPAInterPodVIPAName -protocol $TCP -port $8472 -description "Horizon 7 CPA Inter-Pod VIPA" -EnableInheritance
        $SVHZN7ConnServerInstallReplica = New-NsxService -name $SVHZNConnServerInstallReplicaName -protocol $TCP -port $32111 -description "Horizon 7 View Connection Server Install Replica" -EnableInheritance
        $SVHZN7ConnServer2Enrollment = New-NsxService -name $SVHZNConnServer2EnrollmentName -protocol $TCP -port $32111 -description "Horizon 7 View Connection Server to Enrollment Server View Framework" -EnableInheritance
        $SVHZN7ConnServer2VIDM = New-NsxService -name $SVHZNConnServer2VIDMName -protocol $TCP -port $443 -description "Horizon 7 View Connection Server to VMware Identity Manager Message Bus" -EnableInheritance
        $SVHZN7ConnServer2V4HRMI = New-NsxService -name $SVHZNConnServer2V4HRMIName -protocol $TCP -port $3901 -description "Horizon 7 View Connection Server to V4H RMI" -EnableInheritance
        $SVHZN7ConnServer2V4HBMS = New-NsxService -name $SVHZNConnServer2V4HBMSName -protocol $TCP -port $3101 -description "Horizon 7 View Connection Server to V4H Broker Message Server" -EnableInheritance
        $SVHZN7ConnServer2V4HCMS = New-NsxService -name $SVHZNConnServer2V4HCMSName -protocol $TCP -port $3100 -description "Horizon 7 View Connection Server to V4H Cert Management Server" -EnableInheritance
        $SVHZN7ConnServer22FAMGR = New-NsxService -name $SVHZNConnServer22FAMGRName -protocol $UDP -port $5500 -description "Horizon 7 Default View Connection Server to 2FA Manager  " -EnableInheritance
        ...
        $SVHZN7UAG2ConnServerLogin = New-NsxService -name $SVHZNUAG2ConnServerLoginName -protocol $TCP -port $443 -description "Horizon 7 Unified Access Gateway to View Connection Server Login" -EnableInheritance
        $SVHZN7BEUAG2AgentTCP = New-NsxService -name $SVHZNBEUAG2AgentTCPName -protocol $TCP -port $22443 -description "Horizon 7 Unified Blast Extreme TCP Unified Access Gateway to Horizon Agent " -EnableInheritance
        $SVHZN7BEUAG2AgentUDP = New-NsxService -name $SVHZNBEUAG2AgentUDPName -protocol $UDP -port $22443 -description "Horizon 7 Unified Blast Extreme UDP Unified Access Gateway to Horizon Agent " -EnableInheritance
        $SVHZN7PCOIP2AgentTCP = New-NsxService -name $SVHZNPCOIP2AgentTCPName -protocol $TCP -port $4172 -description "Horizon 7 PCoIP TCP Unified Access Gateway to Horizon Agent" -EnableInheritance
        $SVHZN7PCOIP2AgentUDP = New-NsxService -name $SVHZNPCOIP2AgentUDPName -protocol $UDP -port $4172 -description "Horizon 7 PCoIP UDP Unified Access Gateway to Horizon Agent" -EnableInheritance
        $SVHZN7RDPUAG2Agent = New-NsxService -name $SVHZNRDPUAG2AgentName -protocol $TCP -port $3389 -description "Horizon 7 RDP Unified Access Gateway to Horizon Agent" -EnableInheritance
        $SVHZN7CDRMMRUAG2Agent = New-NsxService -name $SVHZNCDRMMRUAG2AgentName -protocol $TCP -port $9427 -description "Horizon 7 CDR MMR Unified Access Gateway to Horizon Agent" -EnableInheritance
        $SVHZN7USBUAG2Agent = New-NsxService -name $SVHZNUSBUAG2AgentName -protocol $TCP -port $32111 -description "Horizon 7 USB Unified Acess Gateway to Horizon Agent USB Redirection" -EnableInheritance
        $SVHZN7UAG2Agent = New-NsxService -name $SVHZNUAG2AgentName -protocol $TCP -port $443 -description "Horizon 7 Unified Access Gateway to Horizon Agent" -EnableInheritance
        $SVHZN7UAG22FAMGR = New-NsxService -name $SVHZNUAG22FAMGRName -protocol $UDP -port $5500 -description "Horizon 7 Default Unified Access Gateway to 2FA " -EnableInheritance
        $SVHZN7VIDM2ConnServerLDAP = New-NsxService -name $SVHZNVIDM2ConnServerLDAPName -protocol $TCP -port $389 -description "Horizon 7 LDAP VMware Identity Manager to View Connection Server" -EnableInheritance
        $SVHZN7VIDM2ConnServerHTTPS = New-NsxService -name $SVHZNVIDM2ConnServerHTTPSName -protocol $TCP -port $443 -description "Horizon 7 HTTPS VMware Identity Manager to View Connection Server " -EnableInheritance
        $SVHZN7VIDM2VIDMHTTPS = New-NsxService -name $SVHZNVIDM2VIDMHTTPSName -protocol $TCP -port $443 -description "Horizon 7 HTTPS VMware Identity Manager to VMware Identity Manager" -EnableInheritance
        $SVHZN7VIDM2VIDMAudit = New-NsxService -name $SVHZNVIDM2VIDMAuditName -protocol $TCP -port $9300-9400 -description "Horizon 7 VMware Identity Manager to VMware Identity Manager Audit" -EnableInheritance
        $SVHZN7VIDM2SMTP = New-NsxService -name $SVHZNVIDM2SMTPName -protocol $TCP -port $25 -description "Horizon 7 VMware Identity Manager to SMTP Server" -EnableInheritance
        $SVHZN7VIDM2DCLDAP = New-NsxService -name $SVHZNVIDM2DCLDAPName -protocol $TCP -port $389 -description "Horizon 7 LDAP VMware Identity Manager to Domain Controller" -EnableInheritance
        $SVHZN7VIDM2DCKerberosTCP = New-NsxService -name $SVHZNVIDM2DCKerberosTCPName -protocol $TCP -port $88 -description "Horizon 7 Kerberos TCP VMware Identity Manager to Domain Controller" -EnableInheritance
        $SVHZN7VIDM2DCKerberosUDP = New-NsxService -name $SVHZNVIDM2DCKerberosUDPName -protocol $UDP -port $88 -description "Horizon 7 Kerberos UDP VMware Identity Manager to Domain Controller" -EnableInheritance
        $SVHZN7VIDM2DCKerberosPWDTCP = New-NsxService -name $SVHZNVIDM2DCKerberosPWDTCPName -protocol $TCP -port $464 -description "Horizon 7 TCP Kerberos PWD VMware Identity Manager to Domain Controller" -EnableInheritance
        $SVHZN7VIDM2DCKerberosPWDUDP = New-NsxService -name $SVHZNVIDM2DCKerberosPWDUDPName -protocol $UDP -port $464 -description "Horizon 7 UDP Kerberos PWD VMware Identity Manager to Domain Controller" -EnableInheritance
        $SVHZN7VIDM2DCRPC = New-NsxService -name $SVHZNVIDM2DCRPCName -protocol $TCP -port $135 -description "Horizon 7 RPC VMware Identity Manager to Domain Controller" -EnableInheritance
        $SVHZN7VIDM2DNSTCP = New-NsxService -name $SVHZNVIDM2DNSTCPName -protocol $TCP -port $53 -description "Horizon 7 DNS TCP VMware Identity Manager to DNS Server" -EnableInheritance
        $SVHZN7VIDM2DNSUDP = New-NsxService -name $SVHZNVIDM2DNSUDPName -protocol $UDP -port $53 -description "Horizon 7 DNS UDP VMware Identity Manager to DNS Server" -EnableInheritance
        $SVHZN7VIDM2CTXBSTD = New-NsxService -name $SVHZNVIDM2CTXBSTDName -protocol $TCP -port $80 -description "VMware Identity Manager to Citrix integration Broker Standard" -EnableInheritance
        $SVHZN7VIDM2CTXSSL = New-NsxService -name $SVHZNVIDM2CTXSSLName -protocol $TCP -port $443 -description "VMware Identity Manager to Citrix integration Broker SSL" -EnableInheritance
        $SVHZN7VIDM2ThinAppFSSMB = New-NsxService -name $SVHZNVIDM2ThinAppFSSMBName -protocol $TCP -port $445 -description "VMware Identity Manger to ThinApp File Servers SMB" -EnableInheritance
        $SVHZN7VIDM2UpgradeServer = New-NsxService -name $SVHZNVIDM2UpgradeServerName -protocol $TCP -port $443 -description "VMware Identity Manager to Upgrade Server" -EnableInheritance
        $SVHZN7VIDM22FAMGR = New-NsxService -name $SVHZNVIDM22FAMGRName -protocol $UDP -port $5500 -description "VMware Identity Manager Default to 2FA Manager" -EnableInheritance
        $SVHZN7VIDM2AirWatchRESTAPI = New-NsxService -name $SVHZNVIDM2AirWatchRESTAPIName -protocol $TCP -port $443 -description "VMware Identity Manager to VMware AirWatch REST API" -EnableInheritance
        $SVHZN7VIDM2MSSQL = New-NsxService -name $SVHZNVIDM2MSSQLName -protocol $TCP -port $1433 -description "VMware Identity Manager to External MSSQL" -EnableInheritance
        $SVHZN7VIDM2PostgreSQL = New-NsxService -name $SVHZNVIDM2PostgreSQLName -protocol $TCP -port $5432 -description "VMware Identity Manager to Extenal PostgreSQL" -EnableInheritance
        ...
        $SVHZN7APPVMGR2ESXi = New-NsxService -name $SVHZNAPPVMGR2ESXiName -protocol $TCP -port $443 -description "App Volumes Manager to ESXi Hostd" -EnableInheritance
        $SVHZN7APPVMGR2MSSQL = New-NsxService -name $SVHZNAPPVMGR2MSSQLName -protocol $TCP -port $1433 -description "App Volumes Manager Default to MSSQL" -EnableInheritance
        ...
        $SVHZN7Admin2ConnServer = New-NsxService -name $SVHZNAdmin2ConnServerName -protocol $TCP -port $443 -description "Horizon 7 Admin Console to View Connection Server" -EnableInheritance
        $SVHZN7Admin2vCenter = New-NsxService -name $SVHZNAdmin2vCenterName -protocol $TCP -port $443 -description "Horizon 7 Admin Console to vCenter Server" -EnableInheritance
        $SVHZN7Admin2APPVMGR = New-NsxService -name $SVHZNAdmin2APPVMGRName -protocol $TCP -port $443 -description "Horizon 7 Admin Console to App Volumes Manager" -EnableInheritance
        $SVHZN7Admin2VIDM = New-NsxService -name $SVHZNAdmin2VIDMName -protocol $TCP -port $8443 -description "Horizon 7 Admin Console to VMware Identity Manager" -EnableInheritance
        $SVHZN7Admin2V4H = New-NsxService -name $SVHZNAdmin2V4HName -protocol $TCP -port $443 -description "Horizon 7 Admin Console to V4H" -EnableInheritance
        $SVHZN7Admin2UAG = New-NsxService -name $SVHZNAdmin2UAGName -protocol $TCP -port $9443 -description "Horizon 7 Admin Console to Unified Access Gateway" -EnableInheritance
